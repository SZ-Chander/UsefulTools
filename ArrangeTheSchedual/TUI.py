    ...
    def TidyTimeStr(self,Datas):
        ...
        for n_data in OldData:
            if(n_data != ''):
                ...
                n_data = n_data.replace('$通','通').replace('$早','早').replace('$晚','晚')
                if(n_data[0] == '$'):
        ...
        for num, key in enumerate(KeyBox):
            ...
            TimeContent = NewGroup[num].replace('早','').replace('晚','').replace('通','')
...
if __name__ == '__main__':
    ...
    parse.add_argument('-EndTime',type=str,default="2021-2-7")
    arg = parse.parse_args()
    Times = TimeTranser(arg.StartTime,arg.EndTime)
    BaseDir = os.getcwd()
    MessPath = "{}/{}".format(BaseDir,"Mess.txt")
    with open(MessPath,encoding='utf-8') as mess:
        messes = mess.readlines()
